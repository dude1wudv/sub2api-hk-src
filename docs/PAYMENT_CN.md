...
| **支付宝手动到账** | 静态收款码 | 用户扫码后由管理员核对实收金额并确认；仅支持 CNY |
...
手动到账使用后台配置的静态收款码，不调用支付宝 API，也不需要 Webhook。用户可以使用配置的任意有效充值金额（正数且最多两位小数），最终仍受全局最低/最高金额和该服务商实例的单笔限额约束。

手动到账也支持订阅套餐。订阅订单必须选择 CNY 定价的在售套餐，页面会显示套餐应付金额；用户完成转账后，管理员在订单详情中输入实际收款金额，金额必须与订单应付金额精确一致。确认后系统将订单标记为已支付并复用正常的余额充值或订阅权益发放流程。订单过期、已处理或金额不一致时不会重复发放权益。

配置步骤：

1. 在服务商管理中添加 **支付宝手动到账** 实例。
2. 配置该实例使用的静态收款码图片，并设置需要的单笔金额限额。
3. 用户创建充值或订阅订单并扫码付款。
4. 管理员核对收款记录，在管理后台订单详情中确认订单。
...
- **支付宝手动到账**没有支付平台回调；管理员确认前订单不会充值或发放订阅权益，确认时必须输入与订单应付金额一致的 CNY 金额
