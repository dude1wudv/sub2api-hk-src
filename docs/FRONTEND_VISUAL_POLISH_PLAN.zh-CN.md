...
> 状态：历史提案；当前视觉方向已由下述 Precision Console 改版取代。正文保留作为原始设计背景，不代表当前配色要求。
>
> 当前实现：使用现有 `workspace.css` 主题体系，统一冰白／深海军蓝表面、电蓝／青色强调、网格与轨道装饰，以及按钮、输入框、表格、弹窗和导航的视觉反馈；保留三种外观选项及浅深色偏好。首页、认证页面与应用框架同步更新，不改变 Vue 脚本、API、Store、路由、权限、计费或表格数据逻辑。
>
> 本次核验：在实际浏览器查看首页、登录页的桌面及手机浅深色布局、手机注册页和替代外观，并以无认证数据的组件预览查看控制台框架。按要求未运行测试套件；未连接可用后端，不包含登录、支付等业务流程验证。
