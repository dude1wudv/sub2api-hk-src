...
> 状态：历史提案；正文保留作原始设计背景，当前实施以本节 Developer Console 更新为准。
>
> 当前实现：Vue 3 / Vite / TailwindCSS；默认深色，提供浅色和跟随系统。统一中性深色层级、克制的靛蓝强调、紧凑字体与数据表格、230px／68px 侧栏及 60px 顶栏。允许适量技术视觉装饰，优先信息层级、可读性与操作反馈。
>
> 交互：权限感知的 Ctrl/Cmd+K 页面命令搜索；表格列显隐、密度与按用户隔离的查询偏好；已保存筛选；账户详情抽屉；分组路由示意；紧凑用量图表与模型下钻；创建密钥的一次性提示；设置未保存提醒。保留后端接口、权限和计费规则，仅修改前端展示与交互。
>
> 数据边界：现有接口没有提供完整 P95／请求成功率序列、账户事件时间线或全部供应商聚合；未虚构这些数据，也未新增后端接口。日期接口使用日历日，快捷范围准确显示 Today 而非假称滚动 24H。密钥提示仅表示创建展示一次，不承诺后端密钥不可再次读取。
>
> 交付约束：按要求不运行测试套件、构建、lint 或类型检查。仅查看实际页面和无业务数据的布局组件渲染；未连接可用后端，不包含登录、支付、真实流量或完整业务流程验证。
