...
GOLANG_IMAGE="${SUB2API_GOLANG_IMAGE:-golang:1.26.4-alpine}"
...
LOCAL_GOLANG_IMAGE="${SUB2API_LOCAL_GOLANG_IMAGE:-localhost/sub2api-base-golang:1.26.4-alpine}"
...
if [ -n "${SUB2API_DOCKER_BUILD_PROXY:-}" ]; then
    proxy_args+=(
        ...
        --build-arg "http_proxy=${SUB2API_DOCKER_BUILD_PROXY}"
        --build-arg "https_proxy=${SUB2API_DOCKER_BUILD_PROXY}"
        --build-arg "NO_PROXY=${SUB2API_DOCKER_BUILD_NO_PROXY:-localhost,127.0.0.1,host.docker.internal,sub2api-postgres,sub2api-redis,kiro-gateway,mihomo}"
    )
fi
