...
func (Proxy) Fields() []ent.Field {
	return []ent.Field{
		...
		// NOTE: keep these immediately after "status" so the generated ent code
		// (ent/runtime, ent/proxy) field indices stay valid. JP-only fields are
		// appended afterwards and are accessed via raw SQL, not the ent builders.
		...
		// JP local: runtime health / failure-based cooldown tracking (raw SQL only).
