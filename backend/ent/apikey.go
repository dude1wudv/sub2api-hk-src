...
type APIKey struct {
	...
	// Ordered smart routing groups; empty means fixed group
...
func (*APIKey) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case apikey.FieldRoutingGroupIds, apikey.FieldIPWhitelist, apikey.FieldIPBlacklist:
			values[i] = new([]byte)
...
func (_m *APIKey) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case apikey.FieldRoutingGroupIds:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field routing_group_ids", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &_m.RoutingGroupIds); err != nil {
					return fmt.Errorf("unmarshal field routing_group_ids: %w", err)
				}
			}
...
func (_m *APIKey) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("routing_group_ids=")
	builder.WriteString(fmt.Sprintf("%v", _m.RoutingGroupIds))
	builder.WriteString(", ")
