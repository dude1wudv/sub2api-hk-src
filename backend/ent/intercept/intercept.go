...
// The DailyBalanceGrantFunc type is an adapter to allow the use of ordinary function as a Querier.
type DailyBalanceGrantFunc func(context.Context, *ent.DailyBalanceGrantQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f DailyBalanceGrantFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.DailyBalanceGrantQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.DailyBalanceGrantQuery", q)
}

// The TraverseDailyBalanceGrant type is an adapter to allow the use of ordinary function as Traverser.
type TraverseDailyBalanceGrant func(context.Context, *ent.DailyBalanceGrantQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseDailyBalanceGrant) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseDailyBalanceGrant) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.DailyBalanceGrantQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.DailyBalanceGrantQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.DailyBalanceGrantQuery:
		return &query[*ent.DailyBalanceGrantQuery, predicate.DailyBalanceGrant, dailybalancegrant.OrderOption]{typ: ent.TypeDailyBalanceGrant, tq: q}, nil
