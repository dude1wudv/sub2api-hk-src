...
//   - pure quota uses the "quota" placeholder (primary_model is NotEmpty so
//     history/timeline paths do not need a special case)
//   - Grok probe and quota_probe default to the lightweight probe model
func normalizeMonitorPrimaryModel(provider, checkMode, model string) string {
	model = strings.TrimSpace(model)
	checkMode = defaultCheckMode(checkMode)
	if model == "" && checkMode == MonitorCheckModeQuota {
