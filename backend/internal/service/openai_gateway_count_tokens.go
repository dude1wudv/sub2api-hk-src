...
func shouldEstimateOpenAIInputTokensLocally(account *Account) bool {
	if account == nil || account.IsGrok() || account.IsCNProvider() || account.Platform == PlatformMirasim || account.Type == AccountTypeUpstream {
		return true
	}
...
func (s *OpenAIGatewayService) ForwardCountTokensAsAnthropic(
	...
	if account.IsCNProvider() || account.IsOpenCodeGo() || account.Platform == PlatformMirasim {
