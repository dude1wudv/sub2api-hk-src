...
func TestBuildTokenIncentiveStatusMatches497MProgress(t *testing.T) {
	...
	if status.ClaimedBalance != 4 || status.ClaimableBalance != 0 {
	...
	if got := status.Tiers[4]; got.Status != "locked" || got.RemainingTokens != 2_700_000 {
...
func TestBuildTokenIncentiveStatusTotalsMaxRewardAt1B(t *testing.T) {
	...
	if status.ClaimableBalance != 10 || status.MaxBalance != 10 {
