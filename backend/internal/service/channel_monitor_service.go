...
func validateCreateParams(p ChannelMonitorCreateParams) error {
	...
	if p.AccountID != nil && *p.AccountID <= 0 {
		// Create has no existing association to clear. Zero is reserved for the
		// update-only explicit-unbind sentinel; negative IDs are always invalid.
		return ErrChannelMonitorInvalidAccountID
	}
	...
	if usesQuota && p.AccountID == nil {
		return ErrChannelMonitorAccountRequired
	}
	primaryModel := normalizeMonitorPrimaryModel(p.Provider, checkMode, p.PrimaryModel)
	if primaryModel == "" || (checkMode != MonitorCheckModeQuota && primaryModel == MonitorDefaultQuotaModel) {
...
func applyMonitorUpdate(existing *ChannelMonitor, p ChannelMonitorUpdateParams) error {
	...
	if p.AccountID != nil {
		...
		case *p.AccountID < 0:
			return ErrChannelMonitorInvalidAccountID
		case *p.AccountID == 0:
			existing.AccountID = nil // 0 = the sole explicit unbind sentinel
		default:
			id := *p.AccountID
	...
	checkMode := defaultCheckMode(existing.CheckMode)
	if p.PrimaryModel != nil {
		primaryModel := normalizeMonitorPrimaryModel(existing.Provider, checkMode, *p.PrimaryModel)
		if primaryModel == "" || (checkMode != MonitorCheckModeQuota && primaryModel == MonitorDefaultQuotaModel) {
			return ErrChannelMonitorMissingPrimaryModel
		}
