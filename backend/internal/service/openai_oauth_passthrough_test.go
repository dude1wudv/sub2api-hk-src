...
func TestOpenAIGatewayService_OAuthLegacy_CompositeCodexUAUsesCodexOriginator(t *testing.T) {
	...
	require.NoError(t, err)
	require.NotNil(t, upstream.lastReq)
	// Browser-like composite UA is replaced with default Codex UA (codex-tui form);
	// originator is paired from the final UA (issue #3901).
...
func TestOpenAIGatewayService_OAuthPassthrough_ResponseHeadersDropUserVisibleQuotaSignals(t *testing.T) {
	gin.SetMode(gin.TestMode)
...
// Regression (issue #3901): official UAs like codex-tui must be preserved in
// passthrough mode, and originator must be paired from the final UA. The old
// fallback rewrote codex-tui → codex_cli_rs while keeping a mismatched
// client originator, which ChatGPT rejected with 404.
func TestOpenAIGatewayService_OAuthPassthrough_CodexTuiIdentityPreservedAndPaired(t *testing.T) {
	...
	// Client sends a mismatched originator; must be re-paired from final UA.
