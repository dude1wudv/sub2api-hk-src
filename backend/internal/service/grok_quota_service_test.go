...
func TestGrokQuotaServiceProbeUsageStoresHeaders(t *testing.T) {
	...
	require.Equal(t, xai.GrokCLIResponsesUserAgent, upstream.requests[0].Header.Get("User-Agent"))
	require.Equal(t, xai.TokenAuthCLI, upstream.requests[0].Header.Get("X-XAI-Token-Auth"))
	require.Equal(t, xai.GrokCLIAuthenticateResponse, upstream.requests[0].Header.Get("X-AuthenticateResponse"))
