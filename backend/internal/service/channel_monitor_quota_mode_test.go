...
func TestValidateCreateParams_CheckModeMatrix(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name: "create rejects update-only account clear sentinel",
			params: ChannelMonitorCreateParams{
				Provider: MonitorProviderKimi, CheckMode: MonitorCheckModeQuota,
				IntervalSeconds: 60, AccountID: &zeroAccountID,
			},
			wantErr: ErrChannelMonitorInvalidAccountID,
		},
		{
			name: "quota probe rejects virtual quota primary model",
			params: ChannelMonitorCreateParams{
				Provider: MonitorProviderKimi, CheckMode: MonitorCheckModeQuotaProbe,
				Endpoint: "https://api.moonshot.cn", APIKey: "sk", IntervalSeconds: 60,
				AccountID: &accountID, PrimaryModel: MonitorDefaultQuotaModel,
			},
...
// --- Update：account_id 三态与 provider/check_mode 组合校验 ---

func TestApplyMonitorUpdate_AccountBindingAndProviderModeConstraints(t *testing.T) {
	linkedAccountID := int64(9)
	monitor := &ChannelMonitor{
		Provider:  MonitorProviderOpenAI,
	...
	// 省略 AccountID 不更新已有关联。
	require.NoError(t, applyMonitorUpdate(monitor, ChannelMonitorUpdateParams{}))
	require.NotNil(t, monitor.AccountID)
	require.Equal(t, linkedAccountID, *monitor.AccountID)

	// 指向 0 是唯一的显式解绑指令。
	clearAccountID := int64(0)
	require.NoError(t, applyMonitorUpdate(monitor, ChannelMonitorUpdateParams{AccountID: &clearAccountID}))
	require.Nil(t, monitor.AccountID)

	// 负数既不是账号关联也不是清空哨兵，必须被拒绝。
	negativeAccountID := int64(-1)
	require.ErrorIs(t, applyMonitorUpdate(monitor, ChannelMonitorUpdateParams{AccountID: &negativeAccountID}), ErrChannelMonitorInvalidAccountID)

	// Provider 单独更新也必须复核已有 check_mode；antigravity 只能 quota。
	antigravity := MonitorProviderAntigravity
	monitor = &ChannelMonitor{
		Provider:  MonitorProviderOpenAI,
		CheckMode: MonitorCheckModeProbe,
		Endpoint:  "https://api.openai.com",
	}
	require.ErrorIs(t, applyMonitorUpdate(monitor, ChannelMonitorUpdateParams{Provider: &antigravity}), ErrChannelMonitorInvalidCheckMode)
}
