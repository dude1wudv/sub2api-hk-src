...
type fixedCNBalanceUpstream struct {
	response *http.Response
	calls    int
}

func (u *fixedCNBalanceUpstream) Do(_ *http.Request, _ string, _ int64, _ int) (*http.Response, error) {
	u.calls++
	return u.response, nil
}

func (u *fixedCNBalanceUpstream) DoWithTLS(req *http.Request, proxyURL string, accountID int64, accountConcurrency int, _ *tlsfingerprint.Profile) (*http.Response, error) {
...
func TestCNProviderBalanceService_KimiBusinessErrorIsNotTreatedAsZeroBalance(t *testing.T) {
	repo := &fakeCNProbeAccountRepo{account: &Account{
	...
	upstream := &fixedCNBalanceUpstream{response: &http.Response{
		...
		Body:       io.NopCloser(strings.NewReader(`{"code":1007,"message":"invalid balance credential"}`)),
	}}
