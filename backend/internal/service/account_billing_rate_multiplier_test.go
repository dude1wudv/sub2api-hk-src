...
func TestAccount_UserBillingRateMultiplier_DefaultsToOneWhenNil(t *testing.T) {
	var a Account
	require.NoError(t, json.Unmarshal([]byte(`{"id":1,"name":"acc","status":"active"}`), &a))
	require.Nil(t, a.UserRateMultiplier)
	require.Equal(t, 1.0, a.UserBillingRateMultiplier())
}

func TestAccount_UserBillingRateMultiplier_AllowsZero(t *testing.T) {
	v := 0.0
	a := Account{UserRateMultiplier: &v}
	require.Equal(t, 0.0, a.UserBillingRateMultiplier())
}

func TestAccount_UserBillingRateMultiplier_NegativeFallsBackToOne(t *testing.T) {
	v := -1.0
	a := Account{UserRateMultiplier: &v}
	require.Equal(t, 1.0, a.UserBillingRateMultiplier())
}
