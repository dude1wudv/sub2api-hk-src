...
func TestForwardAsChatCompletions_ResponseFailed_PassthroughRule(t *testing.T) {
	...
	require.True(t, strings.Contains(errMsg, "context window") || strings.Contains(errMsg, "SUNM_BAD_REQUEST") || strings.Contains(errMsg, "Request could not be processed"), errMsg)
}
...
func TestForwardAsChatCompletions_ResponseFailed_ErrorCodeRuleMatchesViaSemanticStatus(t *testing.T) {
	...
	msg := gjson.Get(respBody, "error.message").String()
