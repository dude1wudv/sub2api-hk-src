...
//
// 所以成功快照统一带 TTL 缓存（monitorQuotaFetchCacheTTL）以防止打爆上游
// 配额端点；失败快照绝不缓存，确保凭据修复或瞬态上游恢复后下一次调度立即重试。
// 同账号的并发抓取仍由 singleflight 合并为一次上游查询。
...
// ChannelMonitorQuotaFetcher 配额抓取器（仅成功快照带 TTL 缓存；失败不缓存，
...
func (f *ChannelMonitorQuotaFetcher) Fetch(ctx context.Context, accountID int64) *domain.MonitorQuotaSnapshot {
	...
	ch := f.flight.DoChan(key, func() (any, error) {
		// A caller can miss the outer cache check, then wait to enter DoChan until
		// the preceding singleflight has completed. Recheck here so that race does
		// not turn one concurrent successful refresh into two upstream calls.
		...
		// 成功快照才缓存。失败不可被缓存，否则账号凭据修复或上游瞬态恢复后
		// 仍会持续把旧错误写入监控历史。
		if snapshot.Success {
			f.storeSnapshot(accountID, snapshot, time.Now().Add(monitorQuotaFetchCacheTTL))
		}
		return snapshot, nil
	})
	select {
...
func (f *ChannelMonitorQuotaFetcher) fetchCNQuota(ctx context.Context, account *Account, now time.Time) *domain.MonitorQuotaSnapshot {
	...
	snapshot := &domain.MonitorQuotaSnapshot{
		...
		Error:     sanitizeMonitorQuotaSnapshotError(result.Error, ""),
		FetchedAt: now,
	}
	...
	if !snapshot.Success {
		snapshot.Error = sanitizeMonitorQuotaSnapshotError(snapshot.Error, "cn quota probe failed")
	}
	return snapshot
}
...
func (f *ChannelMonitorQuotaFetcher) fetchCNBalance(ctx context.Context, account *Account, now time.Time) *domain.MonitorQuotaSnapshot {
	...
	if !snapshot.Success {
		snapshot.Error = sanitizeMonitorQuotaSnapshotError(snapshot.Error, "cn balance probe failed")
	}
	return snapshot
}
...
func quotaErrorSnapshot(source, message string, now time.Time) *domain.MonitorQuotaSnapshot {
	return &domain.MonitorQuotaSnapshot{
		Source:    source,
		Success:   false,
		Error:     sanitizeMonitorQuotaSnapshotError(message, ""),
		FetchedAt: now,
	}
}

// sanitizeMonitorQuotaSnapshotError is the sole boundary for text persisted
// in quota snapshots, which can be exposed to users when quota display is on.
func sanitizeMonitorQuotaSnapshotError(message, fallback string) string {
	return truncateMessage(sanitizeErrorMessage(firstNonEmpty(message, fallback)))
}
