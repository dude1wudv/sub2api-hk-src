...
type noCNCountTokensUpstream struct {
	HTTPUpstream
	calls int
}

func (u *noCNCountTokensUpstream) Do(_ *http.Request, _ string, _ int64, _ int) (*http.Response, error) {
	u.calls++
	return nil, context.Canceled
}

func TestForwardCountTokensAsAnthropic_CNProvidersEstimateLocally(t *testing.T) {
	gin.SetMode(gin.TestMode)
	body := []byte(`{"model":"kimi-k2","messages":[{"role":"user","content":"hello world"}]}`)

	for _, platform := range []string{PlatformKimi, PlatformZhipu, PlatformDeepseek} {
		t.Run(platform, func(t *testing.T) {
			recorder := httptest.NewRecorder()
			c, _ := gin.CreateTestContext(recorder)
			upstream := &noCNCountTokensUpstream{}
			svc := &OpenAIGatewayService{httpUpstream: upstream}

			err := svc.ForwardCountTokensAsAnthropic(context.Background(), c, &Account{
			...
			require.Zero(t, upstream.calls, "CN count_tokens must not call unsupported upstream endpoints")
		})
	}
}
