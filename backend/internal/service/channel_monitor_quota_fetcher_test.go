...
func TestQuotaFetcher_DoesNotCacheFailureSnapshot(t *testing.T) {
	fetcher, usage, _, _, accounts := newQuotaFetcherTestSetup(t)
	...
	require.Equal(t, 2, usage.getCalls(), "failure snapshots must not be cached")

	fetcher.mu.Lock()
	_, cached := fetcher.cache[4]
	fetcher.mu.Unlock()
	require.False(t, cached, "failure snapshots must not enter the TTL cache")
}
