...
func TestNormalizeOpenAIReasoningEffortForGPT56(t *testing.T) {
	tests := []struct {
		...
		{name: "sol keeps max", raw: "max", model: "gpt-5.6-sol", want: "max"},
		{name: "sol keeps ultra as-is", raw: "ultra", model: "gpt-5.6-sol", want: "ultra"},
		{name: "terra keeps max", raw: "max", model: "openai/gpt-5.6-terra", want: "max"},
		{name: "luna suffix keeps max", raw: "max", model: "gpt-5.6-luna-2026-07-09", want: "max"},
		{name: "non-5.6 keeps max as-is", raw: "max", model: "deepseek-v4-pro", want: "max"},
		{name: "non-5.6 keeps ultra as-is", raw: "ultra", model: "gpt-5.5", want: "ultra"},
	}
...
func TestNormalizeOpenAICodexCompactReasoningEffortKeepsMax(t *testing.T) {
...
func TestNormalizeOpenAICodexCompactReasoningEffortKeepsUltra(t *testing.T) {
	body := []byte(`{"model":"gpt-5.6-sol","input":"compact me","reasoning":{"effort":"ultra","summary":"auto"}}`)

	normalized, changed, err := normalizeOpenAICodexCompactReasoningEffort(body, "gpt-5.6-sol")

	require.NoError(t, err)
	require.False(t, changed)
	require.Equal(t, "ultra", gjson.GetBytes(normalized, "reasoning.effort").String())
}
...
func TestNormalizeOpenAICodexCompactReasoningEffortForAccountScopesCompatibility(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "OpenAI OAuth compact keeps max",
			path:    "/openai/v1/responses/compact",
			account: &Account{Platform: PlatformOpenAI, Type: AccountTypeOAuth},
			want:    "max",
		},
		{
			name:    "OpenAI OAuth responses keeps max",
			path:    "/openai/v1/responses",
			account: &Account{Platform: PlatformOpenAI, Type: AccountTypeOAuth},
			want:    "max",
		},
		{
			name:    "OpenAI API Key compact keeps max",
			path:    "/openai/v1/responses/compact",
			account: &Account{Platform: PlatformOpenAI, Type: AccountTypeAPIKey},
			want:    "max",
		},
		{
			name:    "Grok OAuth compact keeps max",
			path:    "/openai/v1/responses/compact",
...
func TestDeriveOpenAIReasoningEffortFromModel_SolMaxUltra(t *testing.T) {
	require.Equal(t, "max", deriveOpenAIReasoningEffortFromModel("gpt-5.6-sol-max"))
	require.Equal(t, "ultra", deriveOpenAIReasoningEffortFromModel("gpt-5.6-sol-ultra"))
	require.Equal(t, "max", deriveOpenAIReasoningEffortFromModel("gpt-5.5-max"))
	require.Equal(t, "ultra", deriveOpenAIReasoningEffortFromModel("gpt-5.5-ultra"))
}
