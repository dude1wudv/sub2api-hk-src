...
func TestRelay_OnTurnComplete_UsesSubsequentResponseCreateTimeAcrossPricingBoundary(t *testing.T) {
	...
	secondResponseCreateObservedAt := secondTurnAt.Add(250 * time.Millisecond)
	...
	go func() {
		...
		_, _ = Relay(
			...
			RelayOptions{
				...
				TakeNextTurnStartedAt: func() time.Time { return secondTurnAt },
				OnTurnComplete: func(current RelayTurnResult) {
					turns <- current
				},
			},
	...
	clock.Store(secondResponseCreateObservedAt.UnixNano())
	clientConn.readCh <- passthroughTestFrame{
		msgType: coderws.MessageText,
