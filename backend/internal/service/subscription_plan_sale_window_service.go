...
const (
	subscriptionPlanSaleWindowLeaderLockKey = "subscription_plan:sale_window:leader"
	subscriptionPlanSaleWindowLeaderLockTTL = 3 * time.Minute
	subscriptionPlanSaleWindowTimeout       = 30 * time.Second
)

// SubscriptionPlanSaleWindowService persists automatic unlisting after a sale cutoff.
...
func (s *SubscriptionPlanSaleWindowService) runOnce() {
	...
	ctx, cancel := context.WithTimeout(context.Background(), subscriptionPlanSaleWindowTimeout)
	defer cancel()
	count, err := s.entClient.SubscriptionPlan.Update().
		Where(
			subscriptionplan.ForSaleEQ(true),
			subscriptionplan.SaleEndsAtNotNil(),
			subscriptionplan.SaleEndsAtLTE(time.Now()),
		).
		SetForSale(false).
		Save(ctx)
	if err != nil {
		slog.Error("[SubscriptionPlanSaleWindow] failed to unlist expired plans", "error", err)
		return
	}
	if count > 0 {
		slog.Info("[SubscriptionPlanSaleWindow] unlisted expired plans", "count", count)
	}
}
