...
type UpstreamFailoverError struct {
	...
	Reason                 string      // 已清洗的 failover 原因，供日志/错误摘要使用，不应包含凭证或原始请求体
	ForceCacheBilling      bool        // Antigravity 粘性会话切换时设为 true
	RetryableOnSameAccount bool        // 临时性错误（如 Google 间歇性 400、空响应），应在同一账号上重试 N 次再切换
}

func (e *UpstreamFailoverError) Error() string {
	if reason := e.SafeReason(); reason != "" {
		return fmt.Sprintf("upstream error: %d (failover: %s)", e.StatusCode, reason)
	}
	return fmt.Sprintf("upstream error: %d (failover)", e.StatusCode)
}

func (e *UpstreamFailoverError) SafeReason() string {
	if e == nil {
		return ""
	}
	return sanitizeFailoverReason(e.Reason)
}

type failoverReasonRedactor struct {
	pattern     *regexp.Regexp
	replacement string
}

var failoverReasonRedactors = []failoverReasonRedactor{
	{regexp.MustCompile(`(?i)(authorization\s*[:=]\s*)(bearer\s+)?[^\s,;]+`), `${1}[redacted]`},
	{regexp.MustCompile(`(?i)(["']?api[-_ ]?key["']?\s*[:=]\s*["']?)[^"'\s,;}]+(["']?)`), `${1}[redacted]${2}`},
	{regexp.MustCompile(`(?i)(["']?access[-_ ]?token["']?\s*[:=]\s*["']?)[^"'\s,;}]+(["']?)`), `${1}[redacted]${2}`},
	{regexp.MustCompile(`(?i)(["']?refresh[-_ ]?token["']?\s*[:=]\s*["']?)[^"'\s,;}]+(["']?)`), `${1}[redacted]${2}`},
	{regexp.MustCompile(`(?i)(["']?password["']?\s*[:=]\s*["']?)[^"'\s,;}]+(["']?)`), `${1}[redacted]${2}`},
	{regexp.MustCompile(`(?i)(["']?client[-_ ]?secret["']?\s*[:=]\s*["']?)[^"'\s,;}]+(["']?)`), `${1}[redacted]${2}`},
	{regexp.MustCompile(`(?i)(https?://[^/\s:@]+:)[^@\s/]+(@)`), `${1}[redacted]${2}`},
}

func sanitizeFailoverReason(reason string) string {
	reason = strings.TrimSpace(sanitizeUpstreamErrorMessage(reason))
	for _, redactor := range failoverReasonRedactors {
		reason = redactor.pattern.ReplaceAllString(reason, redactor.replacement)
	}
	return truncateString(reason, 512)
}
...
func (s *GatewayService) SelectAccountWithLoadAwareness(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, metadataUserID string, sub2apiUserID int64) (*AccountSelectionResult, error) {
	...
	if s.concurrencyService == nil || !cfg.LoadBatchEnabled {
		...
		for {
			...
			result, err := s.tryAcquireAccountSlot(ctx, account.ID, account.GatewayMaxConcurrency())
			...
			if stickyAccountID > 0 && stickyAccountID == account.ID && s.concurrencyService != nil {
				...
				if waitingCount < cfg.StickySessionMaxWaiting {
					return s.newSelectionResult(ctx, account, false, nil, &AccountWaitPlan{
						...
						MaxConcurrency: account.GatewayMaxConcurrency(),
		...
		if len(routingCandidates) > 0 {
				...
				if containsInt64(routingAccountIDs, stickyAccountID) && !isExcluded(stickyAccountID) {
					...
					if stickyAccount, ok := accountByID[stickyAccountID]; ok {
						...
						if rpmPass { // 粘性会话窗口费用+RPM 检查
							result, err := s.tryAcquireAccountSlot(ctx, stickyAccountID, stickyAccount.GatewayMaxConcurrency())
							if err == nil && result.Acquired {
								// 会话数量限制检查
									...
									if !s.checkAndRegisterSession(ctx, stickyAccount, sessionHash) {
										...
										return &AccountSelectionResult{
											...
											WaitPlan: &AccountWaitPlan{
												...
												MaxConcurrency: stickyAccount.GatewayMaxConcurrency(),
												Timeout:        cfg.StickySessionWaitTimeout,
												MaxWaiting:     cfg.StickySessionMaxWaiting,
											},
										}, nil
			...
			for _, acc := range routingCandidates {
				routingLoads = append(routingLoads, AccountWithConcurrency{
					ID:             acc.ID,
					MaxConcurrency: acc.GatewayLoadConcurrency(),
				})
			}
			...
			if len(routingAvailable) > 0 {
				...
				for _, item := range routingAvailable {
					result, err := s.tryAcquireAccountSlot(ctx, item.account.ID, item.account.GatewayMaxConcurrency())
					if err == nil && result.Acquired {
						// 会话数量限制检查
				...
				for _, item := range routingAvailable {
					...
					return s.newSelectionResult(ctx, item.account, false, nil, &AccountWaitPlan{
						...
						MaxConcurrency: item.account.GatewayMaxConcurrency(),
						Timeout:        cfg.StickySessionWaitTimeout,
						MaxWaiting:     cfg.StickySessionMaxWaiting,
					})
				}
		...
		if accountID > 0 && !isExcluded(accountID) {
			...
			if ok {
				...
				if !clearSticky && platformOK && modelSupported && modelSchedulable && quotaOK && windowCostOK && rpmOK && schedulable {
					result, err := s.tryAcquireAccountSlot(ctx, accountID, account.GatewayMaxConcurrency())
					if err == nil && result.Acquired {
						// 会话数量限制检查
						if !s.checkAndRegisterSession(ctx, account, sessionHash) {
	...
	if err != nil {
		...
		for len(available) > 0 {
			...
			result, err := s.tryAcquireAccountSlot(ctx, selected.account.ID, selected.account.GatewayMaxConcurrency())
			if err == nil && result.Acquired {
				// 会话数量限制检查
	...
	for _, acc := range candidates {
		...
		return s.newSelectionResult(ctx, acc, false, nil, &AccountWaitPlan{
			AccountID:      acc.ID,
			MaxConcurrency: acc.GatewayMaxConcurrency(),
			Timeout:        cfg.FallbackWaitTimeout,
			MaxWaiting:     cfg.FallbackMaxWaiting,
		})
	}
...
func (s *GatewayService) tryAcquireByLegacyOrder(ctx context.Context, candidates []*Account, groupID *int64, sessionHash string, preferOAuth bool) (*AccountSelectionResult, bool, error) {
	...
	for _, acc := range ordered {
		result, err := s.tryAcquireAccountSlot(ctx, acc.ID, acc.GatewayMaxConcurrency())
		if err == nil && result.Acquired {
			// 会话数量限制检查
			if !s.checkAndRegisterSession(ctx, acc, sessionHash) {
				result.ReleaseFunc() // 释放槽位，继续尝试下一个账号
...
func (s *GatewayService) selectAccountForModelWithPlatform(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, platform string) (*Account, error) {
	...
	if len(routingAccountIDs) > 0 {
		...
		for i := range accounts {
			...
			// require_privacy_set is group-scoped; skip without globally disabling the account.
...
// 异步 worker 只接收计费与会话归类所需快照，不持有 ParsedRequest/RequestBodyRef。
...
func buildUsageBillingCommand(requestID string, usageLog *UsageLog, p *postUsageBillingParams) *UsageBillingCommand {
	...
	// GroupID 用于 repo.Apply 在同事务内判定「每日余额」专属分组并执行额度拆分扣费。
...
func (s *GatewayService) buildRecordUsageLog(
	...
	sessionKeyHash, sessionAliasHashes := UsageSessionHashes(input.RequestBody, input.RequestHeaders, result.RequestID)
	usageLog := &UsageLog{
		UserID:                user.ID,
		APIKeyID:              apiKey.ID,
		AccountID:             account.ID,
