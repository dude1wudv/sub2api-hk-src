...
// PlusUsageSummary handles Plus account standard-pricing usage summary.
// GET /api/v1/admin/accounts/plus-usage-summary
func (h *AccountHandler) PlusUsageSummary(c *gin.Context) {
	c.Header("Cache-Control", "no-store")
	c.Header("Pragma", "no-cache")
	summary, err := h.adminService.GetPlusAccountUsageSummary(c.Request.Context())
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
func (h *AccountHandler) Create(c *gin.Context) {
	...
	if req.UserRateMultiplier != nil && *req.UserRateMultiplier < 0 {
		response.BadRequest(c, "user_rate_multiplier must be >= 0")
		return
	}
	...
	result, err := executeAdminIdempotent(c, "admin.accounts.create", req, service.DefaultWriteIdempotencyTTL(), func(ctx context.Context) (any, error) {
		account, execErr := h.adminService.CreateAccount(ctx, &service.CreateAccountInput{
			...
			UserRateMultiplier:    req.UserRateMultiplier,
...
func (h *AccountHandler) BatchCreate(c *gin.Context) {
	...
	executeAdminIdempotentJSON(c, "admin.accounts.batch_create", req, service.DefaultWriteIdempotencyTTL(), func(ctx context.Context) (any, error) {
		...
		for _, item := range req.Accounts {
			...
			if item.UserRateMultiplier != nil && *item.UserRateMultiplier < 0 {
				failed++
				results = append(results, gin.H{
					"name":    item.Name,
					"success": false,
					"error":   "user_rate_multiplier must be >= 0",
				})
				continue
			}
			...
			account, err := h.adminService.CreateAccount(ctx, &service.CreateAccountInput{
				...
				UserRateMultiplier:    item.UserRateMultiplier,
