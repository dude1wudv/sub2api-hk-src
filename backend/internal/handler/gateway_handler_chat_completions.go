...
func (h *GatewayHandler) ChatCompletions(c *gin.Context) {
	...
	if redirectedBody, redirectedModel, redirected, redirectErr := redirectDeprecatedOpenAIModelInBody(body); redirectErr != nil {
		h.chatCompletionsErrorResponse(c, http.StatusBadRequest, "invalid_request_error", "Failed to normalize request model")
		return
	} else if redirected {
		body = redirectedBody
		reqLog.Info("gateway.chat_completions.model_redirected",
