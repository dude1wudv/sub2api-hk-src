...
func (r *dashboardAggregationRepository) RecordUpstreamBalance(ctx context.Context, item service.UpstreamBalanceAccount, observedAt time.Time) error {
	...
	if db, ok := r.sql.(*sql.DB); ok {
		...
		if err := txRepo.RecordUpstreamBalance(ctx, item, observedAt); err != nil {
			_ = tx.Rollback()
			return err
		}
		return tx.Commit()
	...
	if previous != nil && *previous-item.Balance > upstreamBalanceEpsilon {
		if _, err := r.sql.ExecContext(ctx, `
			INSERT INTO upstream_balance_consumptions
				(account_id, group_id, account_name, group_name, previous_balance, current_balance, amount, unit, consumed_at)
			VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
			item.ID, item.GroupID, item.Name, item.GroupName, *previous, item.Balance, *previous-item.Balance, item.Unit, observedAt,
		); err != nil {
			return err
		}
	}
	_, err = r.sql.ExecContext(ctx, `
		INSERT INTO upstream_balance_snapshots
			(account_id, group_id, account_name, group_name, balance, unit, observed_at)
...
func (r *dashboardAggregationRepository) GetUpstreamBalanceConsumptionSummary(ctx context.Context, now time.Time) (service.UpstreamBalanceConsumptionSummary, error) {
	...
	rows, err := r.sql.QueryContext(ctx, `
		SELECT
			...
			COALESCE(SUM(CASE WHEN consumed_at >= $3 AND consumed_at < $4 THEN amount ELSE 0 END), 0),
			COALESCE(SUM(amount), 0)
		FROM upstream_balance_consumptions`,
		last24h, yesterday, today, tomorrow,
	)
	if err != nil {
		return out, err
	}
