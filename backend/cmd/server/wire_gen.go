...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	workbenchIntegrationRepository := repository.NewWorkbenchIntegrationRepository(client)
	workbenchIntegrationService := service.NewWorkbenchIntegrationService(workbenchIntegrationRepository, apiKeyService, configConfig)
	workbenchHandler := handler.NewWorkbenchHandler(workbenchIntegrationService, configConfig)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	openAIQuotaAutoResetService := service.ProvideOpenAIQuotaAutoResetService(accountRepository, openAIQuotaService, rateLimitService, idempotencyCoordinator, auditLogService, settingService, leaderLockCache)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, workbenchHandler, idempotencyCoordinator, idempotencyCleanupService, openAIQuotaAutoResetService)
