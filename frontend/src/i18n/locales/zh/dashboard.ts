export default {
  ...
  keys: {
    ...
    useKeyModal: {
      title: '使用 API 密钥',
      secretPlaceholderNotice: '示例使用占位符以保护密钥隐私；请通过复制按钮获取真实值。',
  ...
  usage: {
    ...
    cacheHitRate: '缓存命中率',
    cacheReadRate: '缓存读取率',
    cacheReadRateShort: '缓存',
    cacheReadRateFormula: '缓存读取 ÷（输入 + 缓存创建 + 缓存读取）',
    outputTps: '输出阶段 TPS',
    outputTpsShort: '输出',
    outputTpsFormula: '输出 Token ÷（总耗时 − 首字时间）；无首字数据时不可计算',
    ...
    statsFailedToLoad: '加载使用统计失败',
    modelsFailedToLoad: '加载模型分布失败',
    chartsFailedToLoad: '加载用量图表失败',
