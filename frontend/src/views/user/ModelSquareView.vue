            ...
            <div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-dark-600 dark:bg-dark-900">
              ...
              <div class="space-y-2">
                ...
                >
                  ...
                  <span>OpenAI</span>
                  <span class="ml-auto text-xs text-gray-500 dark:text-gray-400">{{ platformModelCount('openai') }}</span>
                </button>
                <button
                ...
                >
                  ...
                  <span>Anthropic</span>
                  <span class="ml-auto text-xs text-gray-500 dark:text-gray-400">{{ platformModelCount('anthropic') }}</span>
                </button>
                <button
                ...
                >
                  ...
                  <span>Image</span>
                  <span class="ml-auto text-xs text-gray-500 dark:text-gray-400">{{ platformModelCount('image') }}</span>
                </button>
              </div>
            </div>
...
import type { UserAvailableChannel, UserAvailableGroup, UserSupportedModelPricing } from '@/api/channels'
...
const platformLabel = (platform: string) => platform === 'openai' ? 'OpenAI' : platform === 'anthropic' ? 'Anthropic' : platform === 'image' ? 'Image' : platform
const modelPlatformKey = (platform: string, pricing: UserSupportedModelPricing | null) =>
  pricing?.billing_mode === BILLING_MODE_IMAGE ? 'image' : platform

// Classify groups by platform
const openAIGroups = computed(() => {
  return availableGroups.value.filter(g => g.platform === 'openai')
})

const anthropicGroups = computed(() => {
  return availableGroups.value.filter(g => g.platform === 'anthropic')
})

const imageGroups = computed(() => {
  return availableGroups.value.filter(g => g.platform === 'image')
})
const channelModels = computed(() => {
  const byName = new Map<string, { name: string; platform: string; pricing: UserSupportedModelPricing | null; groupIds: Set<number> }>()
  for (const channel of availableChannels.value) {
    for (const section of channel.platforms) {
      const groupIds = section.groups.map(g => g.id)
      for (const model of section.supported_models) {
        const key = `${modelPlatformKey(section.platform, model.pricing)}:${model.name}`
        const existing = byName.get(key)
        if (existing) {
          groupIds.forEach(id => existing.groupIds.add(id))
          continue
        }
        byName.set(key, {
          name: model.name,
          platform: modelPlatformKey(section.platform, model.pricing),
...
const platformModelCount = (platform: string) => channelModels.value.filter(m => m.platform === platform).length
const totalModelCount = computed(() => channelModels.value.length)
...
const allModels = computed<ModelPricing[]>(() => {
  ...
  return channelModels.value
    .filter(model => visiblePlatforms.includes(model.platform))
    .filter(model => !selectedProvider.value || selectedProvider.value === model.platform)
    .filter(model => selectedGroupId.value === null || model.groupIds.has(selectedGroupId.value))
    .map(model => {
      const { multiplier, label, isPerCall } = getMultiplierInfo(model.platform as 'openai' | 'anthropic' | 'image')
      const pricing = model.pricing
      const scale = (value: number | null | undefined) => (value ?? 0) * multiplier
      const perCall = isPerCall || pricing?.billing_mode === BILLING_MODE_PER_REQUEST
      return {
        name: model.name,
        provider: platformLabel(model.platform),
        input: perCall ? scale(pricing?.per_request_price) : scale(pricing?.input_price),
        output: perCall ? scale(pricing?.per_request_price) : scale(pricing?.output_price),
        cacheRead: perCall ? scale(pricing?.per_request_price) : scale(pricing?.cache_read_price),
        cacheWrite: perCall ? scale(pricing?.per_request_price) : scale(pricing?.cache_write_price),
        pricePerImage: pricing?.billing_mode === BILLING_MODE_IMAGE ? scale(pricing.per_request_price ?? pricing.image_output_price) : undefined,
        groupLabel: label,
        isPerCall: perCall
      }
    })
...
function formatPrice(price: number): string {
  return `$${(price * 1_000_000).toPrecision(10).replace(/\.?0+$/, '')} / 1M tokens`
}

// Format image price as ¥X.XX / 张
function formatImagePrice(price: number): string {
  return `$${price.toPrecision(10).replace(/\.?0+$/, '')} / request`
}
