...
describe('MonitorFormDialog linked account selector', () => {
  ...
  it('clears the virtual model and explicitly unbinds when an existing quota monitor switches to probe', async () => {
    accountsList.mockResolvedValue({ items: [{ id: 9, name: 'bound', platform: 'anthropic' }] })
    const wrapper = mountDialog(makeMonitor({
      provider: 'anthropic',
      check_mode: 'quota',
      account_id: 9,
    ...
    await primaryModel.setValue('claude-test')
  ...
  it('clears the virtual model when a quota monitor switches to quota_probe', async () => {
    accountsList.mockResolvedValue({ items: [{ id: 9, name: 'bound', platform: 'anthropic' }] })
    const wrapper = mountDialog(makeMonitor({
      provider: 'anthropic',
      check_mode: 'quota',
  ...
  it('omits account_id for an unrelated probe edit', async () => {
    const wrapper = mountDialog(makeMonitor({
      provider: 'anthropic',
      check_mode: 'probe',
      account_id: 9,
    ...
    const [, payload] = monitorUpdate.mock.calls[0]
    expect(payload).not.toHaveProperty('account_id')
  })

  it('sends zero after a probe monitor enters and explicitly exits quota mode', async () => {
    const wrapper = mountDialog(makeMonitor({
      provider: 'anthropic',
      check_mode: 'probe',
      account_id: 9,
