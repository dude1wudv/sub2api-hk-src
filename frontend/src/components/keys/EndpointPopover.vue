...
<template>
  <div v-if="allEndpoints.length > 0" class="flex flex-col gap-2 sm:flex-row sm:flex-wrap">
    <div
      v-for="(item, index) in allEndpoints"
      :key="index"
      class="flex min-w-0 items-center gap-1.5 rounded-xl border border-gray-200 bg-white/80 px-3 py-2 text-xs shadow-sm transition-colors hover:border-primary-200 dark:border-dark-600 dark:bg-dark-800/80 dark:hover:border-primary-700"
    >
      <span class="shrink-0 font-medium text-gray-600 dark:text-dark-200">{{ item.name }}</span>
      <span
        v-if="item.isDefault"
      ...
      <div class="group/endpoint relative flex min-w-0 flex-1 items-center gap-1.5">
        <div
          class="pointer-events-none absolute bottom-full right-0 z-20 mb-2 w-[min(24rem,calc(100vw-2rem))] translate-y-1 rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-left opacity-0 shadow-lg ring-1 ring-gray-200/80 transition-[opacity,transform] duration-150 group-hover/endpoint:translate-y-0 group-hover/endpoint:opacity-100 group-focus-within/endpoint:translate-y-0 group-focus-within/endpoint:opacity-100 dark:border-dark-600 dark:bg-dark-900 dark:ring-dark-600/70 sm:left-1/2 sm:right-auto sm:w-max sm:max-w-[24rem] sm:-translate-x-1/2"
        >
          <p
        ...
        <code
          class="min-w-0 flex-1 truncate font-mono text-gray-500 decoration-gray-400 decoration-dashed underline-offset-2 hover:text-primary-600 hover:underline focus:text-primary-600 focus:underline focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-500/40 dark:text-dark-300 dark:decoration-dark-400 dark:hover:text-primary-400 dark:focus:text-primary-400"
          role="button"
          tabindex="0"
        ...
        <button
          type="button"
          class="inline-flex min-h-11 min-w-11 shrink-0 items-center justify-center rounded-lg transition-colors sm:min-h-9 sm:min-w-9 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500/40"
