...
describe('UseKeyModal', () => {
  it('renders GPT-5.6 terra and medium effort in OpenAI Codex config', () => {
  ...
  it('renders GPT-5.6 terra and medium effort in OpenAI Codex WebSocket config', async () => {
    const wrapper = mount(UseKeyModal, {
      props: {
        show: true,
        apiKey: 'sk-test',
  ...
  it('renders GPT-5.4 mini entry in OpenCode config', async () => {
    ...
    expect(codeBlock.text()).toMatch(/"gpt-5\.6-sol"[\s\S]*"max"\s*:\s*\{\}/)
  })
