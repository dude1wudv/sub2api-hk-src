...
function selectCheckMode(mode: CheckMode) {
  ...
  const previousMode = form.check_mode
  form.check_mode = mode
  if (!usesQuotaMode.value) {
    form.account_id = null
    if (previousMode !== CHECK_MODE_PROBE) accountClearRequested.value = true
  } else {
    accountClearRequested.value = false
  }
  // 纯 quota 行的 "quota" 只是后端占位符，切入含探活模式时必须让管理员
  // 选择真实模型，不能把该占位值交给上游。
  if (
    previousMode === CHECK_MODE_QUOTA &&
    usesProbePart.value &&
    form.primary_model.trim() === DEFAULT_QUOTA_MODEL
  ) {
    form.primary_model = ''
  }
}
...
// 仅显式退出 quota 或切换 provider 才向后端发送 account_id=0；普通 probe
// 编辑保留省略语义，避免把历史遗留的可选关联意外解绑。
const accountClearRequested = ref(false)
...
function selectProvider(provider: Provider) {
  ...
  // 关联账号与平台绑定：切换 provider 时显式清空。
  form.account_id = null
  pinnedAccount.value = null
  accountHydrationFailed.value = false
  accountClearRequested.value = true
...
function buildPayload(): CreateParams {
  const requestedPrimaryModel = form.primary_model.trim()
  const probePrimaryModel = requestedPrimaryModel === DEFAULT_QUOTA_MODEL ? '' : requestedPrimaryModel
  return {
    name: form.name.trim(),
    provider: form.provider,
    ...
    primary_model: usesProbePart.value ? probePrimaryModel : DEFAULT_QUOTA_MODEL,
...
async function handleSubmit() {
  ...
  if (
    usesProbePart.value &&
    (!requestedPrimaryModel || requestedPrimaryModel === DEFAULT_QUOTA_MODEL)
  ) {
  ...
  try {
    ...
    if (target) {
      ...
      // 后端 update 的 account_id 有三态：省略/null 不更新、0 显式解绑、正数关联。
      // 不能把 probe 模式的 null 直接透传，否则 quota 监控切回 probe 后会保留旧绑定。
      delete rest.account_id
      const req: UpdateParams = { ...rest }
      const previousUsesQuota = target.check_mode === CHECK_MODE_QUOTA || target.check_mode === CHECK_MODE_QUOTA_PROBE
      const providerChanged = target.provider !== form.provider
      if (usesQuotaMode.value) {
        req.account_id = form.account_id!
      } else if (target.account_id != null && (previousUsesQuota || providerChanged || accountClearRequested.value)) {
        // Clear only for an explicit quota exit or provider switch. A regular
        // probe edit must preserve any existing optional account association.
        req.account_id = 0
      }
