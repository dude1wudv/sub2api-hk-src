<template>
  <BaseDialog :show="show" :title="operation === 'add' ? t('admin.users.deposit') : t('admin.users.withdraw')" width="narrow" @close="$emit('close')">
    <form v-if="user" id="balance-form" @submit.prevent="handleBalanceSubmit" class="space-y-5">
      <div class="flex items-center gap-3 rounded-xl border border-gray-200/80 bg-gray-50/80 p-4 dark:border-dark-700 dark:bg-dark-800/50">
        ...
        <div class="min-w-0 flex-1">
          ...
          <p class="text-xs text-gray-500 dark:text-dark-400">{{ t('admin.users.currentBalance') }}: <span class="font-medium text-gray-900 tabular-nums dark:text-white">${{ formatBalance(user.balance) }}</span></p>
        </div>
      </div>
      <div>
        <label class="input-label">{{ operation === 'add' ? t('admin.users.depositAmount') : t('admin.users.withdrawAmount') }}</label>
        <div class="relative flex gap-2">
          <div class="relative flex-1"><div class="absolute left-3 top-1/2 -translate-y-1/2 font-medium text-gray-400 dark:text-dark-400">$</div><input v-model.number="form.amount" type="number" step="any" min="0" required class="input pl-8 tabular-nums" /></div>
      ...
      <div v-if="form.amount > 0" class="rounded-xl border border-link-200/80 bg-link-50/60 p-4 dark:border-link-900/50 dark:bg-link-950/20"><div class="flex items-center justify-between text-sm"><span class="text-gray-700 dark:text-dark-200">{{ t('admin.users.newBalance') }}:</span><span class="font-bold tracking-tight text-gray-900 tabular-nums dark:text-white">${{ formatBalance(calculateNewBalance()) }}</span></div></div>
    </form>
    <template #footer>
